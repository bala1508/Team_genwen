...
class _EditAttendanceState extends State<EditAttendance> {
  ...
  Future<void> getData() async {
    final res = await Requests.get(
        "http://192.168.54.42:3001/faculty/students/${widget.sem + widget.sec + widget.dept + widget.code}",
        headers: {
          "Authorization": 'Bearer ${await storage.read(key: "jwt")}',
        });
    List k = jsonDecode(res.body)['studentList'];
    print(k.sublist(0, k.length ~/ 2));
    for (var i in k.sublist(0, k.length ~/ 2)) {
      attendanceList.add(i['attended']);
    }
    studentList = k.sublist(k.length ~/ 2);
    setState(() {
      isLoading = false;
    });
  }
